...
public class KThread {
	...
	public static void finish() {
		...
		//Someelse thread called join on this thread
		if (currentThread.wakeUp!=null){
			currentThread.wakeUp.ready();
		}
		//
	...
	private boolean alreadyJoined=false; //Whether this thread joined by another thread
	private KThread wakeUp=null; //Resume thread call join on this

	public void join() {

		//Critical section
		//Prevent multiple threads call (join on a "this" thread or join be called multiple times)
		boolean intStatus=Machine.interrupt().disable();
		...
		Lib.assertTrue(this.alreadyJoined==false);

		this.alreadyJoined=true;

		if(this.status!=statusFinished){
			// System.out.println(curThread.toString());
			//System.out.println(this.toString());
			this.wakeUp=KThread.currentThread();
			KThread.currentThread().sleep();
			System.out.println("ok, we can continue!");
		}
		else{
	...
	KThread child1 = new KThread( new Runnable () {
		public void run() {
		    ...
		    System.out.println("I (heart) Nachos! I am finished");
		}
	    });
	...
	//Without join, all threads can finish without problem
	//Testcase passed!
